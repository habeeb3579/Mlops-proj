...
REFERENCE_DATA_PATH = '/home/habeeb/Mlops-proj/05-monitoring/data/reference.parquet'
CURRENT_DATA_PATH = 'data/green_tripdata_2024-03.parquet'
MODEL_PATH = '/home/habeeb/Mlops-proj/05-monitoring/models/lin_reg.bin'
...
def calculate_and_store_metrics(index, reference_df, current_df, model):
    ...
    quantile_score = float(result['metrics'][3]['value'])
    #share_missing_values = float(result['metrics'][3]['value'])
    ...
    logger.info(f"✅ Quantile metric for {day_start.date()} is {quantile_score}.")


# ------------------------- Prefect Flow ------------------------- #
@flow(name="Evidently homework Workflow")
